...
class CameraMatrix:
    def __init__(self, K, width, height, R, t):
        t = t.reshape((3, 1))
        P = np.matmul(K, np.hstack([R, -np.matmul(R, t)]))
        self.P = P
        self.width = width
        self.height = height


def camera_matrix_project(p_3D, camera):
    p_3D_ = np.hstack([p_3D, np.ones((p_3D.shape[0], 1))])
    p_2D = np.matmul(camera.P, p_3D_.T)
